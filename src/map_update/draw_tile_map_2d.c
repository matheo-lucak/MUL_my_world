                                                        ...
                                                        int y, int x)
{
    ...
    if (win_settings.mouse_tool.hold) {
        ...
        if (sfFloatRect_contains(&hit_box, win_settings.mouse_tool.pos.x, win_settings.mouse_tool.pos.y)) {
            my_printf("Text changed !\n");
            terraformer->tile_map_2d[y][x].rstate.texture = terraformer->textures[1];
